...
00000
📊 114/12/30 專案進度總結 (Project Summary)
專案狀態：地圖定位核心已完成 (Map Core Done)，目前進入功能互動除錯階段 (Interaction Debugging)。

一、 今日達成里程碑 (Achievements)
✅ 點位座標正常：

成功解決 SharePoint 資料連線 (L1b) 問題。

成功解決 ID 抓取問題（使用 StartsWith 克服了 "A" 與 "A棟" 的文字差異）。

確認紅點已準確顯示在對應的房間位置上。

二、 待修復問題 (Pending Issues)
⚠️ 顏色顯示錯誤 (Color Logic)：

現狀：點位目前可能全灰或顏色判斷不準。

原因：Status (狀態) 欄位是「選項 (Record)」格式，與文字比對時發生型別衝突；或是 L3 查驗紀錄與房間 ID 的關聯沒對上。

⚠️ 新增資料報錯 (Form Creation Error)：

現狀：點選紅點要新增查驗單時發生錯誤。

推測原因：

Maps 傳遞參數問題（是否正確把房間 ID 傳給下一頁？）。

下一頁表單 (Form) 的 Item 屬性或 DefaultMode 設定有誤（系統不知道要幫哪個房間新增資料）。

三、 明日工作計畫 (Tomorrow's To-Do)
明天一早開始，我們將依序執行以下修復：
...
確認 L3 資料表是用哪一個欄位來關聯房間 ID (是 ID 還是 查閱欄位？)。

第二步：修復點擊新增功能

檢查圓點的 OnSelect 程式碼：是否正確設定 NewForm(Form_L3)？
...
00000
這份日誌是專為您未來的「導航」而設計的。它不僅記錄了**怎麼做**，更記錄了**為什麼當初那樣做會失敗**。這將成為您最珍貴的專案資產。

---

# 📑 2025/12/31 專案開發終極存檔：建築查驗系統紀錄

### 📅 開發日期：2025 年 12 月 31 日

**目前狀態**：已完成地圖點位連動、多筆缺失存檔、狀態自動判定、地圖動態顏色顯示。

---

## 一、 核心架構：為什麼我們放棄 SubmitForm？

在開發過程中，我們花了大量時間測試 `SubmitForm(Form1)`，但最終決定全面改用 **`Patch`**。

* **冤枉路：** 使用 `SubmitForm` 時，只要資料卡片（DataCard）內部有微小的「循環參考」或 OData 宣告錯誤，就會噴出 `Unexpected character '{'` 或 `解析失敗`。
* **儲存點邏輯：** `Patch` 允許我們**繞過 UI 限制**。即使表單上的 DataCard 顯示錯誤，只要後台變數 `varCurrentPinData` 是正確的，我們就能強行把正確的資料塞進 SharePoint。

---

## 二、 那些「血淚教訓」：避坑指南

### 1. ID 的大小寫敏感 (The ID/Id Trap)

* **教訓**：SharePoint 查閱項（Lookup）在 `Patch` 語法中，內部的 Key 必須是 **`Id`** (小寫 d)，但我們在引用的變數中，系統給的通常是 **`.ID`** (全大寫)。
* **解決**：寫成 `Id: varCurrentPinData.ID`。只要這裡的大小寫寫錯，Power Apps 就會報「引數類型無效」。

### 2. 日期格式的「分鐘」與「月份」

* **教訓**：標題 ID 出現 `2025MM...` 這種亂碼。
* **解決**：在 `Text()` 函數中，月份是 **`mm`** (小寫)，小時是 `hh`。如果寫成 `MM`，Power Apps 可能會混淆。
* **公式參考**：`Text(Now(), "yyyymmddhhmmss")`。

### 3. 表單殘留的「幽靈資料」

* **教訓**：點選 02F 的點位，存進去卻是 05F 的樓層。
* **解決**：這是因為 Form 的緩存（Cache）沒清乾淨。必須執行 **「先 ResetForm，後 Set 變數，再 NewForm」** 的三部曲。

---

## 三、 核心代碼庫 (Restore Point Code)

### 1. 地圖圓圈：觸發與鎖定變數

```powerapps
// 位置：地圖頁面 > Circle > OnSelect
ResetForm(Form1);                          // 清除上一次的殘留
Set(varCurrentPinData, ThisItem);           // 鎖定當前 L1b 點位整筆資料
NewForm(Form1);                             // 初始化新表單介面
Navigate(Screen_Inspection_Form);           // 跳轉

```

### 2. 儲存按鈕：精準 Patch 與自動判斷

```powerapps
// 位置：表單頁面 > 儲存按鈕 > OnSelect
Patch(
    'L3_查驗紀錄',
    Defaults('L3_查驗紀錄'),
    {
        // 文字直接連結變數，不透過 Form UI
        標題: Concatenate(varCurrentPinData.樓層戶別.Value, "-", Text(Now(), "yyyymmddhhmmss")),
        工項: ddl_WorkItem.Selected.標題,
        次工項: ddl_SubItem.Selected.標題,
        階段名稱: "內裝工程",
        缺失描述: DataCardValue9.Text, 

        // 查閱項對齊：Id 用小寫 d，Value 用變數文字
        '樓層戶別': { Id: varCurrentPinData.樓層戶別.Id, Value: varCurrentPinData.樓層戶別.Value },
        '點位關聯': { Id: varCurrentPinData.ID, Value: varCurrentPinData.點位名稱 },
        '缺失內容': { Id: DataCardValue8.Selected.Id, Value: DataCardValue8.Selected.Value },

        // 照片與狀態自動化
        缺失照片: Image4.Image,
        改善照片: Image5.Image,
        狀態: { Value: If(IsBlank(Image5.Image), "待改善", "已完成") }
    }
);
ResetForm(Form1); // 存完洗乾淨
Notify("儲存點：資料已安全寫入", NotificationType.Success);
Back();
...
* **規則**：一個點位若有 10 筆缺失，只要 1 筆是「待改善」，球就是**紅色**。只有 10 筆全都是「已完成」，球才會變**綠色**。
* **公式屬性**：圓圈的 `Fill`

```powerapps
With({ _records: Filter('L3_查驗紀錄', '點位關聯'.Id = ThisItem.ID) },
    If(
        IsEmpty(_records), RGBA(200, 200, 200, 1),              // 灰色：無紀錄
        !IsEmpty(Filter(_records, 狀態.Value = "待改善")),      // 紅色：只要有一筆待改善
        RGBA(255, 0, 0, 1),
        RGBA(0, 176, 80, 1)                                   // 綠色：全數改善完成
    )
)
...
目前的進度已經解決了「輸入端」的所有問題。下次我們將開啟 **`Screen_WorkItem_Summary`** 的統計計算，讓管理員能一眼看出目前的合格率。

**如果您已經將這份總結存好，我們今天就完美收官了！**
...
檢查表單頁面：確認表單能自動帶入目前的「樓層戶別 ID」。

今日的除錯過程非常紮實，地圖終於「活」過來了。請好好休息，我們明天一早繼續完成最後的互動功能！晚安！
